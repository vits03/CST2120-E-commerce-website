  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (var i = 0; i < orders.length; i++) {
        ...
        $.ajax({
          url:
            "../server/getCustomerDetails.php?customerID=" +
            order.customerID.$oid,
          type: "GET",
          success: function (response) {
        ...
        var row =
          ...
          "</td>" +
          '<td class="buttons">' +
          '<a class="button-68" href="customerorderdetails.php?id=' +
          order._id.$oid +
          '" id="edit" role="button">View</a>' +
          '<div class="delete-button">' +
          '<button class="button-68 delete" id="delete" data-id="' +
          order._id.$oid +
          '">Delete</button>' +
          "</div>" +
          "</td>" +
      ...
      $("#sort-price-lth").on("click", function () {
        ...
        rows.sort(function (rowA, rowB) {
          var priceA = parseFloat(
            $(rowA).find("td:eq(3)").text().replace(/\D/g, "")
          );
          var priceB = parseFloat(
            $(rowB).find("td:eq(3)").text().replace(/\D/g, "")
          );
