    ...
    success: function (response) {
      ...
      for (var i = 0; i < products.length; i++) {
        ...
        var row =
          ...
          "</td>" +
          '<td class="buttons">' +
          '<a class="button-68" href="editproduct.php?id=' +
          product._id.$oid +
          '" id="edit" role="button">Edit</a>' +
      ...
      $("#sort-price-low-to-high").on("click", function () {
        ...
        tableRows.sort(function (a, b) {
          var aVal = parseFloat(
            $(a).find("td:nth-child(4)").text().replace("$", "")
          );
          var bVal = parseFloat(
            $(b).find("td:nth-child(4)").text().replace("$", "")
          );
          return aVal - bVal;
        });
        $("#table-body").empty().append(tableRows);
